  ...
  return (
    ...
    >
      <>
        <button
          onClick={handleClose}
          className="modalButton absolute right-5 top-5 !z-40 h-9 w-9 border-none  hover:bg-[#181818] "
        >
          ...
          <div className="absolute bottom-10 flex w-full items-center justify-between px-10">
            <div className="flex space-x-2">
              <button className="flex items-center gap-x-2 rounded bg-white px-8 text-xl font-bold text-black transition hover:bg-[#e6e6e6]">
                <FaPlay className="h-6 w-6 text-black" /> Play
              </button>
            ...
            <button onClick={() => setMuted(!muted)} className="modalButton">
                {muted ? (<FaVolumeXmark className="h-6 w-6" />) : (<FaVolumeHigh className="h-6 w-6" />)}
            </button>
          </div>
        </div>
